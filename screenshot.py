...
def main(url, w, h):
    ...
    settings = {
        "windowclass_rendering_enabled" : True, 
    }
    ...
    cef.Intialize(settings=settings, switches=switches)
    create_browser(browser_settings)
    cef_MessageLoop()
    cef.Shutdown()
    print("Opening your Screenshot with default Application")
...
def check_versions():
    ...
    print("CEF {var} ".format(ver=ver["cef_version"]))
    print("Python {var} {arch} ". format(var=platform.python_version(), arch= platform.architecture()[0]))
...
def create_browser(settings):
    ...
    window_info.SetAsPffscreen(parent_window_hadle)
...
def save_screenshot(browser):
    ...
    if not buffer_string:
        raise Exception("Buffer String was empty because "
        "OnPaint was never called")
...
def exit_app(browser):
    print("Closing and Exiting Application")
    browser.CloserBrowser()
    cef.QuitMessageLoop()
...
class RenderHandler(object):
    ...
    def OnPaint(self, browser, element_type, paint_buffer, **_):
        ...
        if element_type == cef.PET_VIEW():
            ...
            raise Exception("Unsupported element type i OnPaint")
